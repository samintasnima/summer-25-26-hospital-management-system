...
/* ---------------- 5. Live dashboard statistics ----------------
   Dashboards opt in by placing data-stat on their number elements.
   Values are refreshed every 30 seconds without reloading the page.
*/
function refreshDashboardStats() {
    var nodes = document.querySelectorAll('[data-stat]');
    if (!nodes.length) return;
    fetch('index.php?page=ajax&action=dashboard_stats', {credentials:'same-origin'})
      ...
      .then(function(payload){
        ...
        nodes.forEach(function(el){
          ...
          if (Object.prototype.hasOwnProperty.call(data,key)) {
            var value=data[key];
            if (['today_revenue','month_revenue','today_collection'].indexOf(key)>=0) {
